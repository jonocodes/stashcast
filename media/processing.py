...
def prefetch_ytdlp(item, tmp_dir, log_path):
    ...
    if is_html_page:
        ...
        media_url, detected_media_type, extracted_title = extract_media_from_html(
            item.source_url
        )
        ...
        if media_url:
            ...
            existing = (
                MediaItem.objects.filter(
                    webpage_url=original_url, media_type=item.media_type
                )
                .exclude(guid=item.guid)
                .first()
            )
            ...
            item.slug = ensure_unique_slug(
                slug, original_url, existing, item.media_type
            )
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if item.requested_type == MediaItem.REQUESTED_TYPE_AUTO:
                ...
                item.media_type = (
                    ...
                    if has_video
                    else MediaItem.MEDIA_TYPE_AUDIO
                )
            else:
                item.media_type = item.requested_type
            ...
            existing = (
                MediaItem.objects.filter(
                    source_url=item.source_url, media_type=item.media_type
                )
                .exclude(guid=item.guid)
                .first()
            )
            slug = generate_slug(item.title)
            item.slug = ensure_unique_slug(
                slug, item.source_url, existing, item.media_type
            )
...
def download_ytdlp(item, tmp_dir, log_path):
    ...
    def progress_hook(d):
        if d["status"] == "downloading":
            ...
            if "downloaded_bytes" in d and "total_bytes" in d:
                ...
                update_progress(
                    item.guid, MediaItem.STATUS_DOWNLOADING, mapped_progress
                )
    ...
    content_files = [
        f
        for f in files
        if f.suffix in [".mp4", ".mkv", ".webm", ".mp3", ".m4a", ".ogg"]
    ]
    if content_files:
        ...
        write_log(
            log_path, f"Content renamed to: {content_file} ({item.file_size} bytes)"
        )
    ...
    if not thumb_files:
        thumb_files = [
            f for f in files if f.suffix in [".jpg", ".jpeg", ".png", ".webp"]
        ]
    ...
    if thumb_files:
        ...
        shutil.move(str(downloaded_file), str(dest))
        write_log(
            log_path, f"Thumbnail renamed to: thumbnail_temp{downloaded_file.suffix}"
        )
    ...
    if subtitle_files:
        ...
        shutil.move(str(downloaded_file), str(dest))
        write_log(
            log_path, f"Subtitles renamed to: subtitles_temp{downloaded_file.suffix}"
        )
...
def process_files(item, tmp_dir, log_path):
        ...
        if content_file.exists():
            ...
            if item.title in ("content", "downloaded-media", "untitled", None):
                ...
                if meta_title:
                    ...
                    write_log(
                        log_path, f"Updated title from media metadata: {meta_title}"
                    )
                else:
                    ...
                    if filename_title:
                        ...
                        write_log(
                            log_path, f"Updated title from filename: {filename_title}"
                        )
            ...
            if metadata:
                try:
                    ...
                    temp_file = (
                        content_file.parent
                        / f"{content_file.stem}_tmp{content_file.suffix}"
                    )
    ...
    for thumb_file in tmp_dir.glob("thumbnail_temp*"):
        ...
        try:
            ...
            process_thumbnail(
                thumb_file, png_path, logger=lambda msg: write_log(log_path, msg)
            )
    ...
    for sub_file in tmp_dir.glob("subtitles_temp*"):
        ...
        try:
            ...
            process_subtitle(
                sub_file, vtt_path, logger=lambda msg: write_log(log_path, msg)
            )
