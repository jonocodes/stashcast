...
from service.transcode_service import (
...
class TranscodeServiceTest(TestCase):
    ...
    @patch('service.transcode_service.needs_transcode')
    @patch('service.transcode_service.download_direct')
    @patch('service.transcode_service.prefetch')
    @patch('service.transcode_service.choose_download_strategy')
    def test_transcode_direct_mp3_no_transcoding(
        ...
        from service.resolve import PrefetchResult
        ...
        def mock_download_func(url, out_path, logger=None):
            ...
            from service.download import DownloadedFileInfo
    ...
    def test_transcode_download_only(
        ...
        mock_strategy.return_value = 'direct'
        mock_needs_transcode.return_value = True  # Would need transcoding but download_only prevents it
    ...
    @patch('service.transcode_service.transcode_to_playable')
    @patch('service.transcode_service.needs_transcode')
    @patch('service.transcode_service.download_direct')
    @patch('service.transcode_service.prefetch')
    @patch('service.transcode_service.choose_download_strategy')
    def test_transcode_ogg_to_m4a(
        self, mock_strategy, mock_prefetch, mock_download,
        mock_needs_transcode, mock_transcode
    ):
        ...
        def mock_transcode_func(input_path, resolved_type, output_path, **kwargs):
            ...
            from service.process import ProcessedFileInfo
    ...
    @patch('service.transcode_service.needs_transcode')
    @patch('service.transcode_service.download_ytdlp')
    @patch('service.transcode_service.prefetch')
    @patch('service.transcode_service.choose_download_strategy')
    ...
    @patch('service.transcode_service.needs_transcode')
    @patch('service.transcode_service.process_thumbnail')
    @patch('service.transcode_service.download_ytdlp')
    @patch('service.transcode_service.prefetch')
    @patch('service.transcode_service.choose_download_strategy')
    def test_transcode_with_thumbnail(
        self, mock_strategy, mock_prefetch, mock_download_ytdlp, mock_process_thumb, mock_needs_transcode
    ):
    ...
    @patch('service.transcode_service.needs_transcode')
    @patch('service.transcode_service.process_subtitle')
    @patch('service.transcode_service.download_ytdlp')
    @patch('service.transcode_service.prefetch')
    @patch('service.transcode_service.choose_download_strategy')
    def test_transcode_with_subtitle(
        self, mock_strategy, mock_prefetch, mock_download_ytdlp, mock_process_sub, mock_needs_transcode
    ):
    ...
    @patch('service.transcode_service.needs_transcode')
    def test_transcode_verbose_logging(self, mock_needs_transcode):
        """Test that verbose mode produces logs"""
        mock_needs_transcode.return_value = False

        with patch('service.transcode_service.choose_download_strategy') as mock_strategy:
            mock_strategy.return_value = 'direct'

            with patch('service.transcode_service.prefetch') as mock_prefetch:
                from service.resolve import PrefetchResult
                prefetch_result = PrefetchResult()
                prefetch_result.title = 'test'
                prefetch_result.has_audio_streams = True
                ...
                with patch('service.transcode_service.download_direct') as mock_download:
                    def mock_download_func(url, out_path, logger=None):
